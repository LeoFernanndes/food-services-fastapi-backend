...
def seed_data(session_, engine):
    session_.add_all([
        ...
        RecipeCategoryOrmModel(name='dessert'),
        RecipeCategoryOrmModel(name='meat'),
        RecipeOrmModel(
...
def test_get_categories_200(seed_data, client):
    response = client.get("/recipes-management/recipe-categories/")
    expected_result = [
        {
            "id": 1,
            "name": "soup"
...
def test_get_category_200(seed_data, client):
    response = client.get('/recipes-management/recipe-categories/1')
    expected_result = {
        'id': 1,
        'name': 'soup'
    }
...
def test_get_category_404(seed_data, client):
    response = client.get('/recipes-management/recipe-categories/404')
    assert response.status_code == 404


def test_post_category_201(seed_data, client):
    payload = {
        'name': 'beans'
    }
...
def test_put_category_200(seed_data, client):
    payload = {
        'name': 'beans'
    }
...
def test_put_category_404(seed_data, client):
    payload = {
        'name': 'beans'
    }
    response = client.put('/recipes-management/recipe-categories/404', json=payload)
    assert response.status_code == 404


def test_put_category_400(seed_data, client):
    payload = {
        'name': 'meat'
    }
    response = client.put('/recipes-management/recipe-categories/1', json=payload)
...
def test_delete_category_204(seed_data, client):
    response = client.delete('/recipes-management/recipe-categories/3')
    assert response.status_code == 204


def test_delete_category_400(seed_data, client):
    response = client.delete('/recipes-management/recipe-categories/1')
    assert response.status_code == 400
